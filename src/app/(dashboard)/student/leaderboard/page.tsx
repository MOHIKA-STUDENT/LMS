...
export default function StudentLeaderboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="py-12 text-center text-theme-sub animate-pulse">Loading academy leaderboard...</div>
      ) : displayedProfiles.length === 0 ? (
        <div className="p-12 text-center bg-theme-card border border-theme rounded-2xl text-theme-sub shadow-sm">
          No students ranked in this view yet.
        </div>
      ) : (
        <div className="bg-theme-card border border-theme rounded-2xl overflow-hidden shadow-2xl space-y-2">
          {/* Top 3 Podium Highlights */}
          {displayedProfiles.length >= 1 && (
            <div className="p-6 bg-gradient-to-b from-indigo-900/20 to-transparent border-b border-theme grid grid-cols-1 sm:grid-cols-3 gap-4">
              {displayedProfiles.slice(0, 3).map((p, rankIdx) => {
                const colors = [
                  'from-amber-500/20 to-yellow-600/10 border-amber-500/40 text-amber-600 dark:text-amber-300',
                  'from-slate-400/20 to-slate-500/10 border-slate-400/40 text-slate-700 dark:text-slate-200',
                  'from-amber-700/20 to-amber-800/10 border-amber-700/40 text-amber-700 dark:text-amber-400',
                ];
                ...
                return (
                  <div key={p.id} className={`p-4 rounded-xl border bg-gradient-to-b ${colors[rankIdx]} flex flex-col items-center text-center space-y-2 relative bg-theme-card`}>
                    <div className="absolute top-2 left-3 font-mono font-black text-xs opacity-75">
                      #{rankIdx + 1}
                    </div>
                    <Icon className="w-8 h-8" />
                    <div>
                      <h4 className="font-bold text-theme-main text-base">{studentName}</h4>
                      <p className="text-xs text-theme-sub">{p.batch?.name || 'Academy Student'}</p>
                    </div>
                    <div className="font-mono font-black text-lg text-amber-500">{p.points} PTS</div>
                  </div>
                );
              })}
            </div>
          ...
          <div className="overflow-x-auto p-4">
            <table className="w-full text-left text-sm text-theme-main">
              <thead className="text-xs uppercase text-theme-sub font-semibold border-b border-theme">
                <tr>
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-slate-100/60 dark:hover:bg-slate-800/40 transition-colors ${
                        isCurrent ? 'bg-indigo-600/20 font-semibold border-l-4 border-l-indigo-500' : ''
                      }`}
                    >
                      <td className="px-6 py-4 font-mono font-bold text-xs text-theme-sub">
                        #{index + 1}
                      </td>
                      <td className="px-6 py-4 font-semibold text-theme-main flex items-center space-x-3">
                        <div className="w-8 h-8 rounded-full bg-theme-card-sub border border-theme flex items-center justify-center text-xs font-bold text-indigo-500">
                          {studentName.charAt(0).toUpperCase()}
                        </div>
                        <span>{studentName} {isCurrent && '(You)'}</span>
                      </td>
                      <td className="px-6 py-4 text-xs text-theme-sub">
                        {p.batch?.name || 'Unassigned'}
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-amber-500 font-mono">
                        {p.points} PTS
                      </td>
                    </tr>
                  );
